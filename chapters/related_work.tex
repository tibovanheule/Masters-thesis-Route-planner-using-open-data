...
In \glsfmtshort{pt}, different sorts of Queries are possible. Whereas in road networks, we are only interested in the shortest path, we are generally more interested in supporting more complex queries. 
\begin{itemize}
    ...
    \item \glsxtrfull{meat} \cite{dibbelt_intriguingly_2013} also an extension od \glsxtrshort{eat} incorperates uncertainty by considering stochastic delays on the vehicles.
\end{itemize}

We are most interested in \glsxtrshort{eat} queries since most other types are an extension of \glsxtrshort{eat}. This means optimising \glsxtrshort{eat}-problems improves the other query types, too \cite{rojas_melendez_publishing_2022}.  
...
\end{enumerate}
\subsubsection{\glsfmtfull{eat} query}
...
Unlike the previous papers, this paper \cite{benatallah_introducing_2014} acknowledges the existence of Transmodel. Their ontology is based on analysing multiple data models: Transmodel, \glsxtrfull{tpeg}, \glsxtrfull{naptan}, VDV 452, \glsxtrshort{ifopt}, \glsxtrshort{siri} and \glsxtrshort{netex}. Since most concepts are derived from transmodel, is the ontology useable for \glsxtrshort{raptor}. However, the ontology is not directly available.
...
This is an ontology \cite{noauthor_transit_nodate} based on \glsxtrshort{gtfs}, but is not updated since its first issue in 2011. The author also mentions that this has been superseded by linked-\glsxtrshort{gtfs}.
...
Linked Connection\cite{rojas_melendez_publishing_2022, noauthor_linked_nodate} is a framework that puts forward one possible in-between approach compared to data dump and purpose specific \glsxtrshort{api}'s. It is a publishing mechanism to put data out ready to be used by a route planning algorithm. 
